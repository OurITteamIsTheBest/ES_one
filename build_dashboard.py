...
pages.overview = () => {
  ...
  // ==== Executive KPIs ====
  const rev2025 = DATA.pl_yearly['Доход от основной деятельности']['2025'] || 0;
  const growthYoY = rev2025 ? (rev-rev2025)/rev2025*100 : null;
  const avgMonthExp = filterState.months.size ? (-totalExp/filterState.months.size) : 0;
  const cashRunway = avgMonthExp > 0 ? (ost/1000) / avgMonthExp : 0;   // ostatok in ₸, expenses in тыс.
  const dzTotal = (DATA.dz && DATA.dz.line_items) ? DATA.dz.line_items.reduce((s,x)=>s+x.total,0) : 0;
  const headCount = (DATA.org_kz?.length || 0) + (DATA.org_uz?.length || 0);
  const top3ClientsShare = (() => {
    if (!DATA.dz?.line_items?.length) return 0;
    const by = {};
    DATA.dz.line_items.forEach(x => { by[x.buyer] = (by[x.buyer]||0) + x.total; });
    const sorted = Object.values(by).sort((a,b)=>b-a);
    const top3 = sorted.slice(0,3).reduce((s,x)=>s+x,0);
    const total = sorted.reduce((s,x)=>s+x,0);
    return total ? top3/total*100 : 0;
  })();

  // ==== Filial × service revenue matrix ====
  // rows = filials, cols = top 6 services by revenue
  const revenueAtoms = at.filter(a => a[1]==='Доходы' && a[6] > 0);
  const serviceTotals = new Map();
  revenueAtoms.forEach(a => serviceTotals.set(a[4], (serviceTotals.get(a[4])||0) + a[6]));
  const topServices = [...serviceTotals.entries()].sort((a,b) => b[1]-a[1]).slice(0,6).map(x => x[0]);
  const filialTotals = new Map();
  revenueAtoms.forEach(a => filialTotals.set(a[2], (filialTotals.get(a[2])||0) + a[6]));
  const topFilials = [...filialTotals.entries()].sort((a,b) => b[1]-a[1]).slice(0,10).map(x => x[0]);
  const cell = (f, s) => revenueAtoms.filter(a => a[2]===f && a[4]===s).reduce((sum,a) => sum + a[6], 0);
  const shortSvc = s => (s||'').replace('Доход от услуг ', '').replace('Доход от ', '').replace('Услуги ','');

  // ==== P&L: план (PBI 2026) vs факт (PL_ES 2025) ====
  const pl2025 = {
    rev: DATA.pl_yearly['Доход от основной деятельности']['2025'] || 0,
    var: DATA.pl_yearly['Переменные производственные расходы']['2025'] || 0,
    fix: DATA.pl_yearly['Постоянные производственные расходы']['2025'] || 0,
    sale: DATA.pl_yearly['Расходы по реализации']['2025'] || 0,
    adm: DATA.pl_yearly['Административные расходы']['2025'] || 0,
    ebitda: DATA.pl_ebitda['2025'] || 0,
  };
  const pl2026 = {
    rev: DATA.pl_yearly['Доход от основной деятельности']['2026'] || 0,
    var: DATA.pl_yearly['Переменные производственные расходы']['2026'] || 0,
    fix: DATA.pl_yearly['Постоянные производственные расходы']['2026'] || 0,
    sale: DATA.pl_yearly['Расходы по реализации']['2026'] || 0,
    adm: DATA.pl_yearly['Административные расходы']['2026'] || 0,
  ...
  return `
    ...
    <div class="card" style="margin-top:16px">
      ...
      <div class="table-scroll"><table>
        <thead><tr><th style="text-align:left">Филиал</th>${topServices.map(s => `<th style="text-align:right">${shortSvc(s)}</th>`).join('')}<th style="text-align:right">Прочее</th><th style="text-align:right; font-weight:600">Итого</th></tr></thead>
        <tbody>
          ${topFilials.map(f => {
            const rowTotal = filialTotals.get(f) || 0;
            const covered = topServices.reduce((s,svc) => s + cell(f, svc), 0);
            const other = rowTotal - covered;
            return `<tr>
              <td style="text-align:left"><b>${f}</b></td>
              ${topServices.map(s => {
                const v = cell(f, s);
                return `<td class="num" style="${!v?'color:var(--muted)':''}">${v ? fmtCompact(v) : '—'}</td>`;
              }).join('')}
              <td class="num" style="color:var(--muted)">${other > 0 ? fmtCompact(other) : '—'}</td>
              <td class="num" style="font-weight:600">${fmtCompact(rowTotal)}</td>
            </tr>`;
          }).join('')}
          <tr style="background:var(--subtle); font-weight:600">
            <td style="text-align:left">Итого по услуге</td>
            ${topServices.map(s => `<td class="num">${fmtCompact(serviceTotals.get(s)||0)}</td>`).join('')}
            <td class="num">${fmtCompact([...serviceTotals.values()].reduce((s,x)=>s+x,0) - topServices.reduce((s,svc) => s + (serviceTotals.get(svc)||0), 0))}</td>
            <td class="num" style="color:var(--pos)">${fmtCompact([...serviceTotals.values()].reduce((s,x)=>s+x,0))}</td>
          </tr>
        </tbody>
      </table></div>
    </div>
    ...
    <div class="card" style="margin-top:16px">
      <div class="card-head"><h3>P&L · Факт 2025 vs План 2026</h3>
        <span style="color:var(--muted); font-size:11.5px">по PL_ES · не зависит от фильтров · тыс. ₸</span></div>
      <div class="table-scroll"><table>
        <thead><tr><th style="text-align:left">Показатель</th><th>Факт 2025</th><th>План 2026</th><th>Δ млн ₸</th><th>Δ %</th></tr></thead>
        <tbody>
          ${[
            ['Выручка','rev', true],
            ['Переменные расходы','var', false],
            ['Постоянные расходы','fix', false],
            ['Расходы по реализации','sale', false],
            ['Административные расходы','adm', false],
            ['EBITDA (расчёт)','ebitda', true],
          ].map(([label, key, isGood]) => {
            const f2025 = pl2025[key], f2026 = pl2026[key];
            const delta = f2026 - f2025;
            const pct = f2025 ? delta/Math.abs(f2025)*100 : null;
            const goodDelta = isGood ? delta >= 0 : delta <= 0;
            return `<tr>
              <td style="text-align:left"><b>${label}</b></td>
              <td class="num">${fmt(f2025,0)}</td>
              <td class="num">${fmt(f2026,0)}</td>
              <td class="num ${goodDelta?'pos':'neg'}">${(delta>=0?'+':'')}${fmt(delta/1000,1)}</td>
              <td class="num ${goodDelta?'pos':'neg'}">${pct!==null ? (pct>=0?'+':'')+pct.toFixed(1)+'%' : '—'}</td>
            </tr>`;
          }).join('')}
        </tbody>
      </table></div>
    ...
    <div class="footer-note">
      <b>Источники:</b> PBI — детальный бюджет 2026 (${DATA.atoms.length} агрегированных записей).
      PL_ES — многолетний БДР 2020–2026 (2020–2025 факт, 2026 план).
      Остаток на счете — снимок ДС.
      ДЗ — последний недельный срез: <b>${DATA.dz?.source_file || '—'}</b>.
      Оргструктура — только действующие сотрудники.
    </div>
  `;
};
